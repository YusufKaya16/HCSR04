...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	//if interrupt source is Channel - 1
	if(htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1){

		//Rising Edge Detection.
		if(EdgeCapture == 0){

			//Read the Channel - 1 value.
			Capture_rising = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1);
			EdgeCapture = 1;
			//Change Rising Edge -> Falling Edge Detection.
		...
		//Falling Edge Detection.
			...
			//Difference between of the Two Edges
			if(Capture_falling > Capture_rising){
				Difference = Capture_falling - Capture_rising;
			}

			else if(Capture_rising > Capture_falling){
				Difference = (0xFFFF - Capture_rising) + Capture_falling;
			}

			//Distance
			Distance = Difference * (0.034 / 2);
			sprintf(TxBuffer, "%d\n", (int)Distance); // @suppress("Float formatting support")
